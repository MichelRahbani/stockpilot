...
// ── ITEMIZED FIXED BILLS ──
function addBillItem() {
  const container = document.getElementById('fixedBillsItems');
  if(!container) return;
  const row = document.createElement('div');
  row.className = 'bill-item-row';
  row.style.cssText = 'display:grid;grid-template-columns:1fr 100px 28px;gap:6px;align-items:center';
  row.innerHTML = `
    <input class="field-input bill-item-name" placeholder="e.g. Phone bill" style="padding:6px 10px;border:1px solid var(--sp-g100);border-radius:8px;font-family:inherit;font-size:13px;background:var(--sp-white)" />
    <input class="field-input bill-item-amount" type="number" placeholder="$0" min="0" step="10" oninput="updateBillTotal()" style="padding:6px 10px;border:1px solid var(--sp-g100);border-radius:8px;font-family:inherit;font-size:13px;background:var(--sp-white)" />
    <button type="button" onclick="this.closest('.bill-item-row').remove();updateBillTotal()" style="background:none;border:none;color:#ccc;cursor:pointer;font-size:16px;padding:0">✕</button>
  `;
  row.querySelector('.bill-item-amount').addEventListener('input', updateBillTotal);
  container.appendChild(row);
}

function updateBillTotal() {
  const amounts = Array.from(document.querySelectorAll('.bill-item-amount'))
    .map(el => parseFloat(el.value) || 0);
  const total = amounts.reduce((s, v) => s + v, 0);
  const totalEl = document.getElementById('billItemsTotal');
  if(totalEl) {
    totalEl.textContent = total > 0 ? 'Total: $' + total.toFixed(0) : '';
  }
  // Auto-update the fixed bills input if items are present
  if(total > 0) {
    const fixedBillsInput = document.getElementById('fixedBills');
    if(fixedBillsInput) {
      fixedBillsInput.value = total;
      fixedBillsInput.dispatchEvent(new Event('input'));
    }
  }
}
...
function renderSpendingPieChart() {
  ...
  const legend = document.getElementById('spendingPieLegend');
  if(!canvas) return;

  const income = parseFloat(document.getElementById('monthlyIncome')?.value) || 0;
  const fixed = parseFloat(document.getElementById('fixedBills')?.value) || 0;
  const flex = parseFloat(document.getElementById('flexSpending')?.value) || 0;
  ...
  const leftover = Math.max(0, income - fixed - flex - savings - debt);
  ...
  const cx = w * 0.38, cy = h / 2, r = Math.min(cx, cy) * 0.85;
  ...
  if(legend) {
    legend.innerHTML = slices.map(s => `
      <div style="display:flex;align-items:center;gap:6px;font-size:13px">
        ...
        <span>${s.label}: <strong>$${s.value.toLocaleString()}</strong></span>
      </div>
    `).join('');
  }
}
...
const ECON_BACKEND = 'https://stockpilot-production-c94f.up.railway.app';
let _macroCache = null;
let _macroCacheTime = 0;
const MACRO_CACHE_MS = 30 * 60 * 1000; // 30 minutes

async function fetchMacroData() {
  const now = Date.now();
  if (_macroCache && now - _macroCacheTime < MACRO_CACHE_MS) return _macroCache;
  try {
    const res = await fetch(ECON_BACKEND + '/api/macro');
    const data = await res.json();
    _macroCache = data;
    _macroCacheTime = now;
    return data;
  } catch(e) {
    console.log('Macro fetch failed:', e);
    return null;
  }
}

function getMacroValue(macroData, id) {
  if (!macroData?.series) return null;
  const s = macroData.series.find(s => s.id === id);
  return s?.latest?.value ?? null;
}

// ── MACRO PULSE STRIP (Command Center) ──
async function loadMacroPulse() {
  const strip = document.getElementById('macroPulseStrip');
  const insight = document.getElementById('macroPulseInsight');
  if (!strip) return;

  strip.innerHTML = '<span style="font-size:12px;opacity:0.4">Loading...</span>';
  const data = await fetchMacroData();
  if (!data) {
    strip.innerHTML = '<span style="font-size:12px;opacity:0.4">Macro data unavailable</span>';
    return;
  }

  const fed = getMacroValue(data, 'FEDFUNDS');
  const cpi = getMacroValue(data, 'CPIAUCSL');
  const unrate = getMacroValue(data, 'UNRATE');
  const dgs10 = getMacroValue(data, 'DGS10');
  const mortgage = getMacroValue(data, 'MORTGAGE30US');

  const pill = (label, value, unit, color) => value !== null
    ? `<div style="display:flex;align-items:center;gap:5px;background:${color}15;border:1px solid ${color}30;border-radius:20px;padding:4px 10px">
        <span style="font-size:11px;opacity:0.55">${label}</span>
        <span style="font-size:13px;font-weight:700;color:${color}">${value.toFixed(2)}${unit}</span>
       </div>`
    : '';

  const fedColor = fed > 4 ? '#ef4444' : fed > 2 ? '#f59e0b' : '#10b981';
  const cpiColor = cpi > 3 ? '#ef4444' : cpi > 2 ? '#f59e0b' : '#10b981';
  const unColor = unrate > 5 ? '#ef4444' : unrate > 4 ? '#f59e0b' : '#10b981';

  strip.innerHTML = [
    pill('Fed Rate', fed, '%', fedColor),
    pill('Inflation (CPI Index)', cpi, '', cpiColor),
    pill('Unemployment', unrate, '%', unColor),
    pill('10Y Treasury', dgs10, '%', '#3b82f6'),
    pill('30Y Mortgage', mortgage, '%', '#8b5cf6'),
  ].join('');

  // Generate insight
  const insights = [];
  if (fed !== null && fed > 4.5) insights.push('🔴 Fed rate is elevated at ' + fed.toFixed(2) + '% — variable-rate debt is expensive right now.');
  else if (fed !== null && fed < 2) insights.push('🟢 Fed rate is low at ' + fed.toFixed(2) + '% — borrowing costs are favorable.');

  if (unrate !== null && unrate > 5) insights.push('⚠️ Unemployment at ' + unrate.toFixed(1) + '% — consider building a larger emergency fund.');
  else if (unrate !== null && unrate < 4) insights.push('🟢 Job market is tight (' + unrate.toFixed(1) + '% unemployment) — good time to negotiate salary or build savings.');

  if (mortgage !== null && mortgage > 6.5) insights.push('🏠 Mortgage rates at ' + mortgage.toFixed(2) + '% — homebuying costs are high.');
...
// ── ECONOMICS IN CONTEXT (Budget tab) ──
async function loadEconContext() {
  const grid = document.getElementById('econContextGrid');
  if (!grid) return;

  grid.innerHTML = '<div style="padding:20px;text-align:center;opacity:0.4;grid-column:1/-1"><div style="font-size:28px;margin-bottom:8px">⏳</div><div style="font-size:13px">Loading economic data...</div></div>';

  const data = await fetchMacroData();
  if (!data) {
    grid.innerHTML = '<div style="padding:16px;text-align:center;opacity:0.4;grid-column:1/-1;font-size:13px">Could not load macro data. Check your connection.</div>';
    return;
  }
  ...
  const fixed = parseFloat(document.getElementById('fixedBills')?.value) || 0;
  const flex = parseFloat(document.getElementById('flexSpending')?.value) || 0;
  const spending = fixed + flex;

  const card = (icon, title, value, detail, color, bg) => `
    <div style="background:${bg};border:1px solid ${color}30;border-radius:12px;padding:16px">
      <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:${color};margin-bottom:6px">${icon} ${title}</div>
      <div style="font-size:22px;font-weight:700;color:var(--sp-text);margin-bottom:4px">${value}</div>
      <div style="font-size:12px;color:var(--sp-g500);line-height:1.5">${detail}</div>
    </div>`;

  const cards = [];

  // 1. Inflation impact on income
  if (cpi !== null && income > 0) {
    // CPI is an index level, not a % — estimate annual inflation from it
    // FRED CPI index around 300 = ~3% recent inflation approximation
    // Use a simplified approach: if CPI index > 290, inflation is ~3%+
    const approxInflation = cpi > 300 ? 3.5 : cpi > 280 ? 3.0 : cpi > 260 ? 2.5 : 2.0;
    const annualLoss = income * 12 * (approxInflation / 100);
    const monthlyLoss = annualLoss / 12;
    const color = approxInflation > 3 ? '#ef4444' : '#f59e0b';
    const bg = approxInflation > 3 ? '#fef2f2' : '#fffbeb';
    cards.push(card('💸', 'Inflation Impact',
      '-$' + monthlyLoss.toFixed(0) + '/mo',
      `At ~${approxInflation}% inflation, your $${income.toLocaleString()} monthly income loses about $${monthlyLoss.toFixed(0)} in purchasing power each month. Consider asking for a raise or finding ways to grow income.`,
      color, bg
    ));
  }

  // 2. Debt pressure vs Fed rate
  if (fed !== null && debt > 0 && income > 0) {
    const debtPct = (debt / income * 100).toFixed(1);
    const isHighRate = fed > 4.5;
    const color = isHighRate ? '#ef4444' : '#10b981';
    const bg = isHighRate ? '#fef2f2' : '#f0fdf4';
    cards.push(card('🏦', 'Fed Rate vs Your Debt',
      fed.toFixed(2) + '% Fed Rate',
      isHighRate
        ? `Rates are high. Your $${debt.toLocaleString()}/mo debt payments (${debtPct}% of income) are more expensive now. Variable-rate debt like credit cards and HELOCs are especially costly — consider paying them down faster.`
        : `Rates are moderate. Your debt load of ${debtPct}% of income is manageable at current rates. Good time to lock in fixed-rate loans if you have variable debt.`,
      color, bg
    ));
  }

  // 3. Emergency fund target based on unemployment
  if (unrate !== null && spending > 0) {
    const recommendedMonths = unrate > 5 ? 6 : unrate > 4 ? 4 : 3;
    const target = spending * recommendedMonths;
    const currentMonths = emergency > 0 ? (emergency / spending).toFixed(1) : 0;
    const isOk = emergency >= target;
    const color = isOk ? '#10b981' : '#f59e0b';
    const bg = isOk ? '#f0fdf4' : '#fffbeb';
    cards.push(card('🛡️', 'Emergency Fund Target',
      recommendedMonths + ' months recommended',
      `Unemployment is at ${unrate.toFixed(1)}%. ${unrate > 5 ? 'Job market is softer — aim for 6 months.' : 'Job market is healthy — 3-4 months is fine.'} You need $${target.toLocaleString()} (${recommendedMonths}x your $${spending.toLocaleString()}/mo spending). You have ${currentMonths} months covered.`,
      color, bg
    ));
  }

  // 4. 10Y Treasury vs savings rate
  if (dgs10 !== null && savings > 0) {
    const goodSavingsRate = dgs10 > 4;
    const color = goodSavingsRate ? '#10b981' : '#3b82f6';
    const bg = goodSavingsRate ? '#f0fdf4' : '#eff6ff';
    const annualInterest = savings * 12 * (dgs10 / 100);
    cards.push(card('💰', 'Savings Rate Opportunity',
      dgs10.toFixed(2) + '% Treasury Yield',
      goodSavingsRate
        ? `High-yield savings and Treasury yields are above ${dgs10.toFixed(1)}%. If you keep your $${savings.toLocaleString()}/mo savings ($${(savings*12).toLocaleString()}/yr) in a HYSA or T-bills, you could earn ~$${annualInterest.toFixed(0)}/yr in interest.`
        : `Treasury yields are moderate at ${dgs10.toFixed(1)}%. Check if your savings account rate is competitive — many HYSAs are offering ${dgs10.toFixed(1)}%+.`,
      color, bg
    ));
  }

  // 5. Mortgage context (if relevant)
  if (mortgage !== null) {
    const isHigh = mortgage > 6.5;
    const color = isHigh ? '#ef4444' : '#10b981';
    const bg = isHigh ? '#fef2f2' : '#f0fdf4';
    cards.push(card('🏠', '30-Year Mortgage Rate',
      mortgage.toFixed(2) + '%',
      isHigh
        ? `Rates are elevated at ${mortgage.toFixed(2)}%. A $300,000 mortgage would cost ~$${Math.round(300000 * (mortgage/100/12) / (1-Math.pow(1+mortgage/100/12,-360))).toLocaleString()}/mo. Renting may be more cost-effective until rates fall.`
        : `Rates at ${mortgage.toFixed(2)}% are relatively low. If you are considering buying, this is a favorable window compared to recent highs.`,
      color, bg
    ));
  }

  if (cards.length === 0) {
    grid.innerHTML = '<div style="padding:16px;opacity:0.5;font-size:13px;grid-column:1/-1">No economic data available right now.</div>';
  } else {
    grid.innerHTML = cards.join('');
  }
}

// Auto-load macro pulse when command center is visible
window.loadMacroPulse = loadMacroPulse;
window.loadEconContext = loadEconContext;

// Load on page ready
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => {
    loadMacroPulse();
  }, 2000); // delay so other data loads first
});
